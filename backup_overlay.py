...
def genoverlay(nodes,saveas=None):
  ...
  for node in nodes:
    x=node["x"]
    y=node["y"]
    addpatch(x,y,nodesize,None,node,"black",True)
    addpatch(x,y,nodesize,node["title"],node,"white",False)
    
  # addpatch(4,4,nodesize,None,nodes[-1],"black",True)
  # addpatch(4,4,nodesize,"test",node,"white",False)
