...
def addtwoimg(fnam,fnam2,label,caption="",wid="0.9"):
  ...
  q=q.replace("###wid###",str(float(wid)/2))
...
def addnode(**q):
  ...
  # if "hidden" in q.keys():
    # hidden=bool(q["hidden"])
  allnodes.append({"id":nodedex,"label":label,"title":title,"loop":loop,"c":[],"looplen":-1,"loopx":loopx,"loopy":loopy,"loopr":loopr})
...
def addmultiply(**q):
  ...
  return "\n\n".join([inside.replace("???",rep) for rep in repl])
...
def enteroverlay(allnodes,folder,data):
  ...
  genoverlay(allnodes,folder+"\\imgs\\back\\overlay")
  
  
  return data.replace("{{{overlay}}}",read("pattern/overlay.txt"))
...
def entertrafos(data):
  ...
  for node in allnodes:
    ...
    for cc in c:
      ...
      goto=cc["q"]
      ret+=read("pattern\\gramap.txt").replace("###wid###",str(wid)).replace("###px###",str(px)).replace("###py###",str(py)).replace("###goto###","page"+str(goto))+"\n"
